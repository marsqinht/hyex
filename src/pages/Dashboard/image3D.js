      ...
      .map((k, i) => {
        ...
        const vars = Array(bezierSegmentation)
          ...
          .map((c, j) => {
            ...
            const x =
              Math.random() *
              (Math.max(startArea.width, endArea.width) + Math.min(startArea.x, endArea.x));
        ...
        const style = {
          transform: `translate(${Math.random() * startArea.width +
            startArea.x}px, ${Math.random() * startArea.height + startArea.y}px)`,
        };
...
export default class SnowDemo extends React.Component {
  ...
  render() {
    ...
    return <div>{this.state.show && <Snow onEnd={this.onEnd}>{children}</Snow>}</div>;
  }
}
